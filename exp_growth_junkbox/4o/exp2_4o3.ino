...
public:
    ...
    float evaluatePolynomial(const PolynomialFit& fit, float x) const {
...
private:
    ...
    static constexpr float MAX_ACCEPTABLE_ERROR = 0.3f;  // Maximum acceptable normalized error
...
void loop() {
    ...
    if (currentTime - lastUpdateTime >= UPDATE_INTERVAL) {
        ...
        float baseValue = 10.0f * exp(0.2f * time);
        ...
        time += 0.1f + (random(-20, 21) / 100.0f);  // 0.08 to 0.12 increment
