...
std::vector<float> AdvancedPolynomialFitter::fitsvm(std::vector<float>& x, const std::vector<float>& y, int degree,
    ...
    const float C = 5000.0;     // Regularization parameter - higher value for tighter fit
    const float epsilon = 0.1; // Epsilon in epsilon-SVR model - increased for better generalization
    const float tol = 1e-4;    // Tolerance for convergence - slightly relaxed
    const int maxIter = 200;   // Maximum iterations - reduced to prevent overfitting
    
    // Initialize alphas and function values
    std::vector<float> alpha(2*n, 0.0);  // Combined alpha (alpha+ and alpha-)
    std::vector<float> f(n, 0.0);        // Function values
...
int AdvancedPolynomialFitter::optimizePair(
    ...
    float s_total = s1 + s2;  // This sum remains constant for the pair
