...
//                std::vector<float> coeffs = polynomialFitter.fitPolynomial(
            ...
            float growthRate = computeGrowthRate(newCoeffs, timestamps, backwardTimeWindow, forwardTimeWindow);
...
void loop() {
   ...
   // float value = 1 * sin(0.2*time)*exp(0.1 *time)) + random(-80, 80) / 10.0;
    ...
    float time_delta = random(0,+100)/100.0;
    float ride = 1*sin(0.2*time) ;

    if (ride >0) {exp_step += time_delta; 
    } else { exp_step = 0 ;}
    
    value = 2*exp(exp_step*0.1);
    time += time_delta;
