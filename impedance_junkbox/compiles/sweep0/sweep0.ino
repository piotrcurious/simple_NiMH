...
// ------ Pin Definitions ------
const int transistorBasePin =   19;      // Digital output to control NPN transistor base
const int batteryVoltagePin  =  39;      // ADC input for battery negative terminal voltage
const int resistorVoltagePin =  34;      // ADC input across 2.5Ω resistor (shunted by 2000µF cap)
...
void setupFrequencies() {
  ...
  float fStart = 10000.0;
  float fEnd = 500000.0; // 1mhz max
...
float measureAtFrequency(float frequency) {
  const int cycles = 100;  // Number of complete cycles for averaging
