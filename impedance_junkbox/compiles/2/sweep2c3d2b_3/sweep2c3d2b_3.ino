...
void loop() {
    ...
    Serial.println("\n--- Resistance to Plot ---");
    for (int i = 0; i < maxFrequencies; i++) {
        Serial.printf("%.3f ", resistanceToPlot[i]);
    }
    Serial.println("\n---------------------------\n");
    ...
    Serial.println("\n--- Capacitance to Plot ---");
    for (int i = 0; i < maxFrequencies; i++) {
        Serial.printf("%.3e ", capacitanceToPlot[i]);
    }
    Serial.println("\n----------------------------\n");
...
void analyzeTimeDomainData(int freqIndex, int dutyIndex, float vccADC[], float batteryDiffADC[], float resistorVoltageADC[]) {
    ...
    // Estimate Capacitance (from discharge transient - IMPROVED DEBUGGING)
    ...
    if (dischargeStartIndex >= onSamples && dischargeStartIndex < dischargeEndIndex) {
        ...
        Serial.printf("    Freq: %.2f Hz, Duty: %.2f%%, R_est: %.3f, V_start: %.3f, V_mid: %.3f\n",
                      sweptFrequencies[freqIndex], dutyCycles[dutyIndex] * 100, r_est, v_start, v_mid);
        ...
        if (!isnan(r_est) && r_est > 0 && v_start > v_mid && v_mid > batteryVoltage[dischargeEndIndex]) {
            ...
            Serial.printf("    Calculated Tau: %.6f, Capacitance: %.3e F\n", tau, inferredCapacitance[freqIndex][dutyIndex]);
            if (inferredCapacitance[freqIndex][dutyIndex] < 1e-9 || inferredCapacitance[freqIndex][dutyIndex] > 1) { // Filter unrealistic values
                ...
                Serial.println("    Capacitance filtered out.");
            }
        } else {
            Serial.println("    Conditions for capacitance calculation not met.");
        }
    } else {
        Serial.println("    Invalid discharge start or end index.");
    }
