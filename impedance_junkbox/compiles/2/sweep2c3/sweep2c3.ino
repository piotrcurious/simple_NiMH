...
void loop() {
  ...
  for (int i = 0; i < frequencySteps; i++) {
    ...
    // Calculate complex voltages (scaling factor applied later)
    std::complex<double> vcc_amplitude = vcc_complex / (samplesPerPulse / 2.0) * (3.3 / 4095.0) * 2.0; // VCC has divider
    std::complex<double> diff_amplitude = diff_complex / (samplesPerPulse / 2.0) * (3.3 / 4095.0);
    std::complex<double> resistor_amplitude = resistor_complex / (samplesPerPulse / 2.0) * (3.3 / 4095.0);
