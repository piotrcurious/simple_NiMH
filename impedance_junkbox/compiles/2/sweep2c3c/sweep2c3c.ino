...
const int endFrequency = 100; // Limiting for now to manage data size
const int frequencySteps = 8;   // Reduced for testing
...
// Data Logging Arrays (Time Domain)
const int maxFrequencies = frequencySteps;
float sweptFrequencies[maxFrequencies];
float timeDomainVCC[maxFrequencies][numDutyCycles][samplesPerCycle];
float timeDomainBatteryDiff[maxFrequencies][numDutyCycles][samplesPerCycle];
float timeDomainResistor[maxFrequencies][numDutyCycles][samplesPerCycle];
...
void analyzeTimeDomainData(int freqIndex, int dutyIndex);
...
void loop() {
  ...
  for (int freqIndex = 0; freqIndex < frequencySteps; freqIndex++) {
    ...
    for (int dutyIndex = 0; dutyIndex < numDutyCycles; dutyIndex++) {
      ...
      for (int sampleIndex = 0; sampleIndex < samplesPerCycle; sampleIndex++) {
        ...
        timeDomainVCC[freqIndex][dutyIndex][sampleIndex] = analogRead(vccVoltagePin);
        timeDomainBatteryDiff[freqIndex][dutyIndex][sampleIndex] = analogRead(batteryVoltagePin);
        timeDomainResistor[freqIndex][dutyIndex][sampleIndex] = analogRead(resistorVoltagePin);
      ...
      analyzeTimeDomainData(freqIndex, dutyIndex);
    }
  ...
  int dutyCycleToPlotR = 1; // Example: 50% duty cycle
  ...
  plotGraph("Inferred Resistance vs. Frequency (50% Duty)", sweptFrequencies, resistanceToPlot, frequencySteps, "Frequency (Hz)", "Resistance (Ohm)");
  delay(5000);

  // Plot Inferred Capacitance vs. Frequency for a specific duty cycle
  int dutyCycleToPlotC = 1; // Example: 50% duty cycle
  float capacitanceToPlot[maxFrequencies];
  for (int i = 0; i < maxFrequencies; i++) {
    capacitanceToPlot[i] = inferredCapacitance[i][dutyCycleToPlotC];
  }
  plotGraph("Inferred Capacitance vs. Frequency (50% Duty)", sweptFrequencies, capacitanceToPlot, frequencySteps, "Frequency (Hz)", "Capacitance (F)");
  delay(5000);
...
void analyzeTimeDomainData(int freqIndex, int dutyIndex) {
  ...
  for (int i = 0; i < samplesPerCycle; i++) {
    vcc[i] = timeDomainVCC[freqIndex][dutyIndex][i] * (3.3 / 4095.0) * 2.0;
    batteryDiff[i] = timeDomainBatteryDiff[freqIndex][dutyIndex][i] * (3.3 / 4095.0);
    resistorVoltage[i] = timeDomainResistor[freqIndex][dutyIndex][i] * (3.3 / 4095.0);
