...
void loop() {
  ...
  for (int i = 0; i < frequencySteps; i++) {
    ...
    // Calculate RMS values (for a square wave fundamental, RMS = Amplitude / sqrt(2))
    float vccVoltageRMS = (vccVoltageAmplitude * 2.0) / sqrt(2.0); // Multiply by 2 for the 1:1 divider
    float batteryDiffVoltageRMS = batteryDiffVoltageAmplitude / sqrt(2.0);
    float resistorVoltageRMS = resistorVoltageAmplitude / sqrt(2.0);
    float batteryVoltageRMS = vccVoltageRMS - batteryDiffVoltageRMS;

    // Calculate current and impedance
    float omega = 2.0 * PI * frequency;
    float parallelImpedanceMagnitude = shuntResistance / sqrt(1.0 + pow(omega * shuntResistance * shuntCapacitance, 2));
    float currentRMS = resistorVoltageRMS / parallelImpedanceMagnitude;
    ...
    Serial.printf("  VCC Voltage (RMS): %.3f V\n", vccVoltageRMS * sqrt(2.0)); // Print the actual RMS of the fundamental
    Serial.printf("  Battery Diff Voltage (RMS): %.3f V\n", batteryDiffVoltageRMS * sqrt(2.0)); // Print the actual RMS of the fundamental
