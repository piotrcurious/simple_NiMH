...
void analyzeTimeDomainData(int freqIndex, int dutyIndex, float vccADC[], float batteryDiffADC[], float resistorVoltageADC[]) {
    ...
    // Estimate Capacitance (from initial slope of voltage during ON)
    int capacitanceStartIndex = 5;
    int capacitanceEndIndex = min(capacitanceStartIndex + 10, onSamples - 1);
