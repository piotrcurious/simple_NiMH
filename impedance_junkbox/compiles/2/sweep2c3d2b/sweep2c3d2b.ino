...
void analyzeTimeDomainData(int freqIndex, int dutyIndex, float vccADC[], float batteryDiffADC[], float resistorVoltageADC[]) {
    ...
    // Estimate ESR (from voltage jump at the beginning of ON pulse)
    int esrStartIndex = 5; // Look at a few samples after the start
    int esrBaselineEndIndex = 2; // Look at a few samples before the start
    if (esrStartIndex < onSamples && esrBaselineEndIndex >= 0) {
        float currentChange = current[esrStartIndex] - (offSamples > 0 ? current[onSamples + esrBaselineEndIndex] : 0);
        float voltageChange = batteryVoltage[esrStartIndex] - (offSamples > 0 ? batteryVoltage[onSamples + esrBaselineEndIndex] : vcc[esrBaselineEndIndex]);
    ...
    // Estimate Capacitance (from discharge transient)
    int dischargeStartIndex = onSamples - 5; // Start of the OFF period
    int dischargeEndIndex = samplesPerCycle - 10; // End of the OFF period
    if (dischargeStartIndex >= 0 && dischargeStartIndex < dischargeEndIndex) {
        float v_start = batteryVoltage[dischargeStartIndex];
        float v_end = batteryVoltage[dischargeEndIndex];
        ...
        if (!isnan(r_est) && r_est > 0 && v_start > v_end) {
            // Simplified approximation: assuming exponential decay
            // v(t) = V0 * exp(-t / RC)
            // ln(v(t)/V0) = -t / RC
            // C = -t / (R * ln(v(t)/V0))
            if (v_start > 0 && v_end > 0) {
                inferredCapacitance[freqIndex][dutyIndex] = -dt / (r_est * log(v_end / v_start));
                if (inferredCapacitance[freqIndex][dutyIndex] < 0 || inferredCapacitance[freqIndex][dutyIndex] > 1) { // Filter out unrealistic values
