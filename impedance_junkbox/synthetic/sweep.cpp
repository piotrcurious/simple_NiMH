...
const int numCyclesPerBurst = 5; // Number of cycles in each burst
const int burstDelayMs = 50;    // Delay between bursts

// Synthetic Frequencies to Sweep
const std::vector<float> burstFrequencies = {100, 500, 1000, 2000, 5000, 10000, 20000};
...
void generateBurstPulse(float frequency);
...
void loop() {
  ...
  tft.setTextSize(1);
  tft.setCursor(0, 0);
  tft.println("Inferring RLC (Synthetic Frequency Sweep)...");
...
void generateBurstPulse(float frequency) {
  ...
  for (int i = 0; i < numCyclesPerBurst; ++i) {
...
void inferRLC() {
  ...
  // Perform synthetic frequency sweep
  for (float frequency : burstFrequencies) {
    tft.printf("Testing Frequency: %.0f Hz...\n", frequency);
    generateBurstPulse(frequency);
    ...
    if (avgCurrent > 1e-6) {
      ...
      Serial.printf("Freq: %.0f Hz, Avg V: %.3f V, Avg I: %.3f A, |Z|: %.3f Ohm\n", frequency, avgVoltage, avgCurrent, avgVoltage / avgCurrent);
    } else {
      Serial.printf("Freq: %.0f Hz, Current too low.\n", frequency);
    }
    delay(100);
  }
