...
void measureInternalResistance() {
    ...
    std::vector<float> consecutive_internal_resistances;
    std::vector<float> unloaded_voltages_history;
    std::vector<unsigned long> unloaded_voltage_timestamps;

    int num_pairs = MAX_RESISTANCE_POINTS / 2; // Aim for this many pairs
    if (num_pairs < 1) {
    ...
    int min_duty_cycle = 0;
    float measurable_current_threshold = 0.01f; // Adjust as needed
    float current_at_min_dc = 0.0f;

    for (int dc = 10; dc <= 255; dc += 5) { // Start from a small value and increment
        dutyCycle = dc;
        ...
        if (current >= measurable_current_threshold) {
            min_duty_cycle = dc;
            current_at_min_dc = current;
            Serial.printf("Minimal duty cycle found: %d, Current: %.3f A\n", min_duty_cycle, current_at_min_dc);
            break;
        }
        if (dc == 255 && current < measurable_current_threshold) {
    ...
    if (min_duty_cycle == 0) {
        Serial.println("Error: Minimal duty cycle not found.");
        return;
    }

    Serial.println("Generating duty cycle pairs...");
    std::vector<std::pair<int, int>> duty_cycle_pairs;
    int high_dc = 255;
    int low_dc = min_duty_cycle;

    for (int i = 0; i < num_pairs; ++i) {
        if (high_dc < low_dc) break; // Ensure high_dc is always greater or equal

        // Check if current at low_dc is still measurable, adjust if needed
        dutyCycle = low_dc;
        analogWrite(pwmPin, low_dc);
        delay(2000);
        float voltage_low_check, current_low_check;
        double temp1_low_check, temp2_low_check, tempDiff_low_check;
        float t1_millivolts_low_check;
        getThermistorReadings(temp1_low_check, temp2_low_check, tempDiff_low_check, t1_millivolts_low_check, voltage_low_check, current_low_check);

           printThermistorSerial(temp1_low_check, temp2_low_check, tempDiff_low_check, t1_millivolts_low_check, voltage_low_check, current_low_check);
            updateTemperatureHistory(temp1_low_check, temp2_low_check, tempDiff_low_check, voltage_low_check, current_low_check);
            prepareTemperaturePlot();
            plotTemperatureData();
            displayTemperatureLabels(temp1_low_check, temp2_low_check, tempDiff_low_check, t1_millivolts_low_check, voltage_low_check, current_low_check);

            tft.setTextColor(TFT_WHITE);
            tft.setTextSize(2);
            tft.setCursor(PLOT_X_START + 20, PLOT_Y_START + PLOT_HEIGHT / 2 - 10);
            tft.print("generating pairs");
            tft.setCursor(PLOT_X_START + 5, PLOT_Y_START + PLOT_HEIGHT / 2 - 30);
            tft.printf("progress: %.1f ", ((float)i/num_pairs)*100.0);
 

        if (current_low_check < measurable_current_threshold && low_dc < high_dc) {
            low_dc += 5; // Increment low duty cycle by a small step
            Serial.printf("Adjusting low duty cycle to %d due to low current.\n", low_dc);
            // Re-check after adjustment (optional, but might be good to ensure it's measurable)
            dutyCycle = low_dc;
            analogWrite(pwmPin, low_dc);
            delay(2000);
            getThermistorReadings(temp1_low_check, temp2_low_check, tempDiff_low_check, t1_millivolts_low_check, voltage_low_check, current_low_check);
            ...
            if (current_low_check < measurable_current_threshold && low_dc >= high_dc) {
                ...
                break; // Stop generating pairs if we can't get a measurable low current
            } else if (current_low_check < measurable_current_threshold) {
                Serial.printf("Warning: Still low current at adjusted low duty cycle %d (%.3f A).\n", low_dc, current_low_check);
            }
        } else if (low_dc > 255) {
        ...
        duty_cycle_pairs.push_back({low_dc, high_dc});
        high_dc -= (255 - min_duty_cycle) / num_pairs; // Decrease high duty cycle
        if (high_dc < min_duty_cycle) high_dc = min_duty_cycle + 5; // Ensure high_dc remains above min_dc
    }

    Serial.printf("Generated %zu duty cycle pairs.\n", duty_cycle_pairs.size());
    ...
    // Measure loaded/unloaded rint using the generated pairs (optional, can be done with individual steps as well)
    Serial.println("\n--- Measuring Internal Resistance (Loaded/Unloaded) ---");
    for (const auto& pair : duty_cycle_pairs) {
    ...
    for (const auto& pair : duty_cycle_pairs) {
        int dc_low = pair.first;
        int dc_high = pair.second;
        ...
        if (current_high > current_low + 0.01f) { // Ensure a meaningful current difference
            ...
            if (resistanceDataCountPairs < MAX_RESISTANCE_POINTS) {
                ...
                internalResistanceDataPairs[resistanceDataCountPairs][1] = -1.0f; // Indicate invalid measurement
                resistanceDataCountPairs++;
            }
