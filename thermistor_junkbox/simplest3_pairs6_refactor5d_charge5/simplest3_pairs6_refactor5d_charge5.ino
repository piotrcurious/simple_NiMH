...
int findOptimalChargingDutyCycle() {
  ...
  float initialUnloadedVoltage = initialUnloadedData.voltage;
    ...
    MHElectrodeData data = measureMHElectrodeVoltage(highDC);
    targetVoltage = initialUnloadedVoltage + (data.loadedVoltage - initialUnloadedVoltage) * MH_ELECTRODE_RATIO;

  while (highDC - lowDC > CHARGE_CURRENT_STEP * 2) {
    int midDC = (lowDC + highDC) / 2;

    MHElectrodeData data = measureMHElectrodeVoltage(midDC);
    // Recalculate target voltage based on the *initial* unloaded voltage
//    targetVoltage = initialUnloadedVoltage + (data.loadedVoltage - initialUnloadedVoltage) * MH_ELECTRODE_RATIO;
// do not recalculate on each step... 

    float currentVoltageDifference = abs(data.loadedVoltage - targetVoltage);

    Serial.printf("Binary Search - Duty Cycle: %d, Unloaded: %.3fV, Loaded: %.3fV, Target: %.3fV, Diff: %.3fV, Current: %.3fA\n",
                  midDC, data.unloadedVoltage, data.loadedVoltage, targetVoltage, currentVoltageDifference, data.current);

    if (data.loadedVoltage < targetVoltage) {
  ...
  for (int dc = max(MIN_CHARGE_DUTY_CYCLE, optimalDutyCycle - CHARGE_CURRENT_STEP * 3);
    ...
    Serial.printf("Fine-tuning - Duty Cycle: %d, Loaded: %.3fV, Target: %.3fV, Diff: %.3fV, Current: %.3fA\n",
                  dc, data.loadedVoltage, targetVoltage, currentVoltageDifference, data.current);
