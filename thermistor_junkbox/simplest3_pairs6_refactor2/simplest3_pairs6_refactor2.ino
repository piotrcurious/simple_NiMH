...
const float MEASURABLE_CURRENT_THRESHOLD = 0.03f; // Adjust as needed (30mA)
const int MIN_DUTY_CYCLE_START = 10;
const int MAX_DUTY_CYCLE = 255;
const int DUTY_CYCLE_INCREMENT = 5;
...
const float MIN_CURRENT_DIFFERENCE_FOR_PAIR = 0.03f;
...
// Step 1: Find the minimal duty cycle for measurable current
int findMinimalDutyCycle() {
    Serial.println("Finding minimal duty cycle for measurable current...");
    for (int dc = MIN_DUTY_CYCLE_START; dc <= MAX_DUTY_CYCLE; dc += DUTY_CYCLE_INCREMENT) {
        MeasurementData data = takeMeasurement(dc,STABILIZATION_DELAY_MS);
        if (data.current >= MEASURABLE_CURRENT_THRESHOLD) {
            Serial.printf("Minimal duty cycle found: %d, Current: %.3f A\n", dc, data.current);
            stopLoad(); // Turn off load after finding
            return dc;
        }
        stopLoad();
        delay(500);
...
// across highDc current increments
std::vector<std::pair<int, int>> generateDutyCyclePairs(int minDutyCycle) {
    Serial.println("Generating duty cycle pairs (improved for linear current spacing)...");
    ...
    if (maxCurrent <= minCurrent) {
        Serial.println("Warning: Maximum current is not greater than minimum current. Cannot ensure linear spacing.");
        // Fallback to the original linear duty cycle spacing
        ...
        for (int i = 0; i < numPairs; ++i) {
            ...
            highDc -= (float)(MAX_DUTY_CYCLE - minDutyCycle) / numPairs;
            if (highDc < minDutyCycle) highDc = minDutyCycle + 3;
        }
    ...
    int lowDc = minDutyCycle;
    float previousHighCurrent = maxCurrent;
    ...
    for (int i = 0; i < numPairs; ++i) {
        ...
        // Search for a high duty cycle that produces a current close to the target
        // Start the search from the previous high duty cycle (or MAX initially) downwards
        for (int highDc = previousHighDc; highDc >= minDutyCycle; highDc--) {
            MeasurementData highData = takeMeasurement(highDc,STABILIZATION_PAIRS_FIND_DELAY_MS);
            processThermistorData(highData, "Searching High DC");
            float currentDifference = std::abs(highData.current - targetHighCurrent);
            ...
            // Optimization: If the current is already below the target, we can stop searching downwards
            if (highData.current <= targetHighCurrent) {
                break;
            }
        }
        ...
        if (currentHighDc < lowDc) currentHighDc = lowDc + 1; // Ensure highDc is not lower than lowDc
...
//                      i, lowDc, currentHighDc, targetHighCurrent, actualHighData.current);
    ...
    Serial.printf("Generated %zu duty cycle pairs (for linear current spacing).\n", pairs.size());
    return pairs;
}
