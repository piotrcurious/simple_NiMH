...
#define THERMISTOR_PIN_1 34 // Analog pin for Thermistor 1 (Bottom in ratiometric config)
#define THERMISTOR_PIN_2 35 // Analog pin for Thermistor 2 (Top in ratiometric config)

// Thermistor parameters - ADJUST THESE BASED ON YOUR THERMISTOR DATASHEET
const double THERMISTORNOMINAL     = 10000.0;  // Resistance at 25 degrees C (Ohms) - Nominal Resistance at 25C (298.15K)
const double TEMPERATURENOMINAL    = 25.0;     // Temperature for nominal resistance (degrees C) - Nominal Temperature 25C (298.15K)
const double BCOEFFICIENT         = 3950.0;   // Beta coefficient (from datasheet) - Beta value
const double SERIESRESISTOR        = 10000.0;  // Series resistor value in voltage divider circuit (Ohms) - Not used in ratiometric config, but kept for thermistor 1 reading
const double VCC_MILLIVOLTS       = 3300.0;   // Supply voltage in millivolts - VCC
...
#define GRAPH_COLOR_DIFF_RATIO TFT_ORANGE
...
float diff_ratio_values[PLOT_WIDTH];
...
double readThermistorRelative(int bottomThermistorPin, double topThermistorTemperatureCelsius, int numSamples);
double readThermistorDifference(int bottomThermistorPin, double topThermistorTemperatureCelsius, int numSamples);
double readThermistorDifference_Ratiometric(int bottomThermistorPin, int numSamples); // New ratiometric difference function
...
// Improved Function to read thermistor temperature with enhanced precision and error handling - Relative to Top Thermistor Temperature
...
// Function to read temperature difference between two thermistors - Original Method
...
// Function to read temperature difference using Ratiometric approach - New Refined Method
// This method calculates the temperature difference based on the ratio of voltages across the thermistors in a ratiometric configuration.
// It uses an approximation that is valid for small temperature differences around the nominal temperature of the thermistors.
double readThermistorDifference_Ratiometric(int bottomThermistorPin, int numSamples = 16) {
    ...
    // Calculate the ratio of resistances R_bottom / R_top based on voltage divider principle
    double ratio_R = averageAnalogValue / (VCC_MILLIVOLTS - averageAnalogValue);

    if (ratio_R <= 0) { // Avoid log of non-positive value
        return std::numeric_limits<double>::quiet_NaN();
    }

    // Approximated temperature difference calculation based on ratiometric ratio
    // Valid for small temperature differences and average temperature close to nominal temperature.
    // Derivation is based on linear approximation of the Beta equation around TEMPERATURENOMINAL.
    double tempDiff_kelvin = (log(ratio_R) / BCOEFFICIENT) * pow((TEMPERATURENOMINAL + 273.15), 2);
    return tempDiff_kelvin; // Temperature difference in Celsius is the same as in Kelvin
}
...
void setup() {
    ...
    // Draw plot frame and axes - Removed drawing axes to maximize plot area
...
void loop() {
    // --- Quantization Error Analysis (Optional - Uncomment for raw ADC readings) ---
    /*
    Serial.print("Raw ADC Readings - T1: ");
    for (int i = 0; i < 5; ++i) {
        Serial.print(analogReadMilliVolts(THERMISTOR_PIN_2)); Serial.print(", ");
        delay(10);
    }
    Serial.print("  T2: ");
    for (int i = 0; i < 5; ++i) {
        Serial.print(analogReadMilliVolts(THERMISTOR_PIN_1)); Serial.print(", ");
        delay(10);
    }
    Serial.println();
    delay(4000); // Longer delay to observe raw readings
    continue; // Skip temperature calculations and plotting for raw reading analysis
    */

    // Read temperatures from thermistors
    double temp1 = readThermistor(THERMISTOR_PIN_2, 32); // Thermistor 1 (Top) connected to THERMISTOR_PIN_2 - Increased averaging for T1
    double temp2 = readThermistorRelative(THERMISTOR_PIN_1,temp1, 32); // Thermistor 2 (Bottom) connected to THERMISTOR_PIN_1, series resistor replaced with "top" thermistor - Increased averaging for T2
    double tempDiff = readThermistorDifference(THERMISTOR_PIN_1, temp1, 32); // Calculate difference using T1 as top temperature and reading T2 - Increased averaging for original diff
    double tempDiff_ratio = readThermistorDifference_Ratiometric(THERMISTOR_PIN_1, 32); // Calculate difference using new ratiometric method - Increased averaging for ratio diff
    ...
    Serial.print(", Diff (T1-T2) [Original]: ");
    if (isnan(tempDiff)) Serial.print("Error"); else Serial.printf("%.2f °C", tempDiff);
    Serial.print(", Diff (T1-T2) [Ratio]: ");
    if (isnan(tempDiff_ratio)) Serial.print("Error"); else Serial.printf("%.2f °C", tempDiff_ratio);
    Serial.println(" °C");
    ...
    for (int i = 0; i < PLOT_WIDTH - 1; i++) {
        ...
        if (!isnan(diff_ratio_values[i]) && !isnan(diff_ratio_values[i + 1])) { // Check for valid temperature values (not NaN)
            int y_diff_ratio_prev = PLOT_Y_START + PLOT_HEIGHT - (int)mapf(diff_ratio_values[i], MIN_DIFF_TEMP, MAX_DIFF_TEMP, 0, PLOT_HEIGHT); // Scale diff with fixed range
            int y_diff_ratio_current = PLOT_Y_START + PLOT_HEIGHT - (int)mapf(diff_ratio_values[i + 1], MIN_DIFF_TEMP, MAX_DIFF_TEMP, 0, PLOT_HEIGHT);
            tft.drawLine(PLOT_X_START + i, y_diff_ratio_prev, PLOT_X_START + i + 1, y_diff_ratio_current, GRAPH_COLOR_DIFF_RATIO);
        }
    }
    ...
    tft.setCursor(PLOT_X_START, LABEL_Y_START + 3 * label_line_height); // Fourth label line
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.print("Diff (T1-T2)[Ratio]: ");
    if (!isnan(tempDiff_ratio)) {
        tft.printf("%.2f C", tempDiff_ratio);
    } else {
        tft.print("Error");
    }
    tft.setTextColor(GRAPH_COLOR_DIFF_RATIO, TFT_BLACK);
    tft.print(" [Orange]");
