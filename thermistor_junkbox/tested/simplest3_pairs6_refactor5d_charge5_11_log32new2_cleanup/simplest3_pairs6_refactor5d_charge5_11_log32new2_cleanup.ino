...
#define TEST_DUTY_CYCLE 64           // Initial test duty cycle value
...
const float MH_ELECTRODE_RATIO = 0.55f;              // Target ratio for MH electrode voltage 
...
float estimateTempDiff(float voltageUnderLoad, float voltageNoLoad, 
                         ...
                         float ambientTempC, uint32_t currentTime,uint32_t lastChargeEvaluationTime) {
...
static const float DEFAULT_CELL_MASS_KG       = 0.011f;   // ~11 g (AAA NiMH ballpark)
...
static const float DEFAULT_CONVECTIVE_H       = 4.0f;     // W/(m^2·K) free convection typical small cell
...
std::pair<int, int> pixelToGrid(int x, int y, int plotAreaWidth, int plotAreaHeight, int marginX, int marginYTop) {
    ...
    int row = static_cast<int>((static_cast<float>(y - marginYTop) / plotAreaHeight) * GRID_SIZE);
    int col = static_cast<int>((static_cast<float>(x - marginX) / plotAreaWidth) * GRID_SIZE);
    #ifdef DEBUG_LABELS
    //Serial.print("Pixel X: "); Serial.print(x); Serial.print(", Y: "); Serial.print(y); Serial.print(" -> Grid Row: "); Serial.print(row); Serial.print(", Col: "); Serial.println(col);
    #endif
    return {std::min(GRID_SIZE - 1, std::max(0, row)), std::min(GRID_SIZE - 1, std::max(0, col))};
}

// Function to map grid indices to pixel coordinates (center of the cell)
...
int calculateVerticalPosition(int yInitial, int textHeight, LabelVerticalPlacement placement) {
    switch (placement) {
        ...
        case LabelVerticalPlacement::ABOVE:
            return yInitial - textHeight - 2; // Add some extra padding
        case LabelVerticalPlacement::BELOW:
            return yInitial + 2;          // Add some extra padding
        default:
            return yInitial - textHeight / 2; // Default to center
    }
}
...
//fixme            float estimatedDiff = estimateTempDiff(logEntry.voltage, logEntry.voltage, logEntry.current, regressedInternalResistancePairsIntercept, logEntry.ambientTemperature);
            float estimatedDiff = currentTempDiff + 3.0; // fixed +2C because now estimatediff requires timestamps to calculate difference. 
            // fixme - whole function needs be re-written to parse log entry differently
...
//            irPairs_Min = std::fmin(irPairs_Min, logEntry.internalResistanceLoadedUnloaded); 
    ...
    // Plot data and update the score grid
    for (size_t i = 2; i < chargeLog.size(); ++i) { //2 because of timestamp history needed
        int x1 = marginX + (chargeLog[i - 1].timestamp - startTime) * timeScaleX;
        int x2 = marginX + (chargeLog[i].timestamp - startTime) * timeScaleX;

        // Helper function to draw line and update grid
        auto drawLineAndUpdateGrid = [&](int _x1, int _y1, int _x2, int _y2, uint16_t color) {
            tft.drawLine(_x1, _y1, _x2, _y2, color);
            // Update grid score - simplified: increment score for cells the line passes through
            // More robust line rasterization algorithm could be used here if needed
            ...
            for (int step = 0; step <= std::max(dx, dy); ++step) {
                ...
                if (currentX == _x2 && currentY == _y2) break;
            }
        };
       ...
       // float estimatedDiffPrev = estimateTempDiff(chargeLog[i - 1].voltage, chargeLog[i - 1].voltage, chargeLog[i - 1].current, regressedInternalResistancePairsIntercept, chargeLog[i - 1].ambientTemperature);
        float estimatedDiffPrev = estimateTempDiff(chargeLog[i-1].voltage, chargeLog[i-1].voltage, chargeLog[i-1].current, regressedInternalResistancePairsIntercept, chargeLog[i-1].ambientTemperature,chargeLog[i-1].timestamp,chargeLog[i-1-1].timestamp,chargeLog[i-1].batteryTemperature);

        float thresholdValuePrev = MAX_TEMP_DIFF_THRESHOLD + estimatedDiffPrev;
        float estimatedDiffCurr = estimateTempDiff(chargeLog[i].voltage, chargeLog[i].voltage, chargeLog[i].current, regressedInternalResistancePairsIntercept, chargeLog[i].ambientTemperature,chargeLog[i].timestamp,chargeLog[i-1].timestamp,chargeLog[i].batteryTemperature);
        //currentTime,lastChargeEvaluationTime,temp2
...
#define LABEL_LINE_LENGTH 30
    ...
    auto createLabel = [&](const std::string& name, float value, float minValue, float maxValue, uint16_t color) {
        ...
        label.y = plotYPixel;
        #ifdef DEBUG_LABELS
        Serial.print("Creating label: "); Serial.print(name.c_str()); Serial.print(", Value: "); Serial.print(value); Serial.print(", Initial Y (Pixel): "); Serial.println(label.y);
        #endif
        ...
        label.minValue = minValue; // Store min value
        label.maxValue = maxValue; // Store max value
    ...
    std::srand(std::time(nullptr)); // Seed random number generator
        ...
        #ifdef DEBUG_LABELS
        Serial.print("Checking if label '"); Serial.print(label.text.c_str());
        Serial.print("' can be placed at Grid Row: "); Serial.print(gridRow);
        Serial.print(", Col: "); Serial.println(gridCol);
        #endif
        ...
        #ifdef DEBUG_LABELS
        Serial.print("gridPixelYCenter: "); Serial.println(gridPixelYCenter);
        Serial.print("textY: "); Serial.println(textY);
        Serial.print("label.y: "); Serial.println(label.y);
        #endif
        int labelTop = textY - labelPadding; // Add padding
        int labelBottom = textY + label.textHeight + labelPadding; // Add padding
        int labelLeft = textX - labelPadding; // Add padding
        int labelRight = textX + label.textWidth + labelPadding; // Add padding

        #ifdef DEBUG_LABELS
        Serial.print("Label Pixel Bounds (with padding): Top="); Serial.print(labelTop); Serial.print(", Bottom="); Serial.print(labelBottom);
        Serial.print(", Left="); Serial.print(labelLeft); Serial.print(", Right="); Serial.println(labelRight);
        #endif

        // Determine the range of grid cells that the label overlaps
        ...
        #ifdef DEBUG_LABELS
        Serial.print("Checking Grid Range: Rows ["); Serial.print(startRow); Serial.print(", "); Serial.print(endRow);
        Serial.print("], Cols ["); Serial.print(startCol); Serial.print(", "); Serial.print(endCol); Serial.println("]");
        #endif
    ...
    auto markLabelAsPlaced = [&](const Label& label, int gridRow, int gridCol) {
        ...
        #ifdef DEBUG_LABELS
        Serial.print("Marking Grid Range: Rows ["); Serial.print(startRow); Serial.print(", "); Serial.print(endRow);
        Serial.print("], Cols ["); Serial.print(startCol); Serial.print(", "); Serial.print(endCol); Serial.println("] with score: "); Serial.println(labelMarkingScore);
        #endif
    ...
    auto drawGridSquare = [&](int row, int col, uint16_t color) {
        int squareSize = 5; // Small size for visibility
...
//        tft.fillRect(x, y, squareSize, squareSize, color);
    ...
    auto placeLabelAndLine = [&](Label& label, bool tryLeftFirst) {
        ...
        // Determine the approximate grid row for the label's Y position
        std::pair<int, int> initialGridPos = pixelToGrid(marginX, label.y, plotAreaWidth, plotAreaHeight, marginX, marginYTop);
        #ifdef DEBUG_LABELS
        Serial.print("Trying to place label '"); Serial.print(label.text.c_str());
        Serial.print("' at initial Grid Row: "); Serial.print(initialGridPos.first);
        Serial.println("");
        #endif
        if (initialGridPos.first == -1) return false;

        int startRow = initialGridPos.first;
        int endRowUp = std::max(0, startRow);
        int endRowDown = std::min(GRID_SIZE - 1, startRow);
        bool placed = false;
        int searchRadius = 0;
        ...
        while (!placed && searchRadius < GRID_SIZE / 2) {
            ...
            auto tryRow = [&](int row) {
                if (row >= 0 && row < GRID_SIZE) {
                    ...
                    auto tryPlacementInRow = [&](int col) {
                        #ifdef DEBUG_LABELS
                        Serial.print("Trying row: "); Serial.print(row); Serial.print(", column: "); Serial.println(col);
                        #endif
                        ...
                        int gridPixelYCenter = gridPixelPos.second; // Center Y
                        int textX;
                        int newTextY;
                        ...
                        if (col < GRID_SIZE / 2) {
                            ...
                            newTextY = gridPixelYCenter - label.textHeight / 2; // Adjust for top alignment
                        }
                        label.x = textX;
                        label.lineY = gridPixelPos.second;

                        int labelTop = newTextY;
                        int labelBottom = newTextY + label.textHeight;

                        // Temporarily update label.y for canPlaceLabel check
           ...
           //                 tft.drawLine(label.lineStartX, label.lineY, label.lineEndX, label.lineY, label.color);
            ...
            // draw AFTER update
                            ...
                            #ifdef DEBUG_LABELS
                            Serial.print("Label '"); Serial.print(label.text.c_str()); Serial.print("' repositioned to Row: "); Serial.println(row);
                            #endif

                            return true;
                        } else {
                            //drawGridSquare(row, col, TFT_RED);
                            return false;
                        }
                    };

                    if (tryLeftFirst) {
                        for (int col = startColLeft; col <= endColLeft; ++col) {
                            if (tryPlacementInRow(col)) return true;
                        }
                        for (int col = endColRight; col >= startColRight; --col) {
                            if (tryPlacementInRow(col)) return true;
                        }
                    } else {
                        for (int col = endColRight; col >= startColRight; --col) {
        ...
        #ifdef DEBUG_LABELS
        Serial.print("Could not place label: ");
        Serial.println(label.text.c_str());
        #endif
        return false;
    };
    ...
    for (auto& label : labels) {
        placeLabelAndLine(label, true); // Try left side first
    }
