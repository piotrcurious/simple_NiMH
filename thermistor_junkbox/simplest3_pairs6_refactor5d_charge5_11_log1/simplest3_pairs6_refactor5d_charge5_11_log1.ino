...
void drawChargePlot(bool autoscaleX = true, bool autoscaleY = true) {
    if (chargeLog.empty()) {
        tft.fillScreen(TFT_BLACK);
        tft.setTextColor(TFT_WHITE);
    ...
    for (size_t i = 1; i < chargeLog.size(); ++i) {
        ...
        // Current
        int y1 = marginYTop + scaleValue(chargeLog[i - 1].current, currentMin, currentMax);
        int y2 = marginYTop + scaleValue(chargeLog[i].current, currentMin, currentMax);
        tft.drawLine(x1, y1, x2, y2, TFT_MAGENTA);

        // Voltage
        y1 = marginYTop + scaleValue(chargeLog[i - 1].voltage, voltageMin, voltageMax);
        y2 = marginYTop + scaleValue(chargeLog[i].voltage, voltageMin, voltageMax);
        tft.drawLine(x1, y1, x2, y2, TFT_YELLOW);

        // Duty Cycle
        y1 = marginYTop + scaleValue(chargeLog[i - 1].dutyCycle, dutyCycleMin, dutyCycleMax);
        y2 = marginYTop + scaleValue(chargeLog[i].dutyCycle, dutyCycleMin, dutyCycleMax);
        ...
        y1 = marginYTop + scaleValue(currentTempDiffPrev, tempDiffMin, tempDiffMax);
        y2 = marginYTop + scaleValue(currentTempDiffCurr, tempDiffMin, tempDiffMax);
        ...
        y1 = marginYTop + scaleValue(thresholdValuePrev, estTempDiffThresholdMin, estTempDiffThresholdMax);
        y2 = marginYTop + scaleValue(thresholdValueCurr, estTempDiffThresholdMin, estTempDiffThresholdMax);
        tft.drawLine(x1, y1, x2, y2, TFT_SKYBLUE);

        // Internal Resistance Loaded/Unloaded 
        y1 = marginYTop + scaleValue(chargeLog[i - 1].internalResistanceLoadedUnloaded, irLU_Min, irLU_Max);
        y2 = marginYTop + scaleValue(chargeLog[i].internalResistanceLoadedUnloaded, irLU_Min, irLU_Max);
        tft.drawLine(x1, y1, x2, y2, TFT_ORANGE);

        // Internal Resistance Pairs 
        y1 = marginYTop + scaleValue(chargeLog[i - 1].internalResistancePairs, irPairs_Min, irPairs_Max);
        y2 = marginYTop + scaleValue(chargeLog[i].internalResistancePairs, irPairs_Min, irPairs_Max);
    ...
    // Draw Legend
    int legendY = marginYTop + plotAreaHeight + 1;
    int legendX = marginX;
    int colorSize = 9;
    int textOffset = 12;

    tft.fillRect(legendX, legendY, colorSize, colorSize, TFT_GREEN);
...
void handleIRCommand() {
      ...
      //debug        
        ...
        case RemoteKeys::KEY_SOURCE:{        
          drawChargePlot(); // Display the charging process log graph
          delay(10000); // wait 10 seconds
        break;
        }    
