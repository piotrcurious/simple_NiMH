...
const int HIGH_CHARGE_DUTY_CYCLE = 200; // Duty cycle to simulate high charging current
...
void chargeBattery() {
    ...
    // 2. Apply a high charging current and measure loaded voltage
    ...
    delay(CHARGE_STABILIZATION_DELAY_MS * 5); // Allow more time for stabilization under high current
    ...
    highCurrentLoadedVoltage = highLoadData.voltage;
    Serial.printf("Voltage under high charging current: %.3f V\n", highCurrentLoadedVoltage);

    // 3. Calculate the initial target "half voltage"
    ...
    while (true) {
        ...
        if (millis() - lastEvaluationTime >= CHARGE_EVALUATION_INTERVAL_MS) {
            ...
            // d. Find the charging current that keeps the MH electrode voltage at the target value
